...
func categorizeSchemaChanges(oldSchema, newSchema []Schema) ChangeLog {
	...
	// NOTE: Currently the feature in not worked on!!
	...
	// Converted the []Schema to maps.
	oldSchemaMap := make(map[string][]SchemaField)
	for _, schema := range oldSchema {
		oldSchemaMap[schema.SchemaName] = schema.SchemaFields
	}
	newSchemaMap := make(map[string][]SchemaField)
	for _, schema := range newSchema {
		newSchemaMap[schema.SchemaName] = schema.SchemaFields
	}

	for _, schemaName := range oldSchema {
		// Convert SchemaFields to maps for easy lookup
		oldFieldsMap := make(map[string]string)
		for _, field := range oldSchemaMap[schemaName.SchemaName] {
			oldFieldsMap[field.Name] = field.DataType
		}

		newFieldsMap := make(map[string]string)
		for _, field := range newSchemaMap[schemaName.SchemaName] {
			newFieldsMap[field.Name] = field.DataType
		}

		// Detect Field Creation and Updates
